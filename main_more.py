...
def evaluate(model, train_mat, test_mat, config, logger, device):
    ...
    with torch.no_grad():
        ...
        evals = Eval()
        m = evals.evaluate_item(train_mat[users, :], test_mat[users, :], users, model, device, topk=50)
    return m
    ...
    for epoch in range(config.epoch):
        ...
        for batch_idx, data in enumerate(train_dataloader):
            ...
            if sampler.__class__.__name__ in ["two_pass_rank", "two_pass_weight_rank"]:
                ...
                loss = model.loss_function(neg_rat, prob_neg, pos_rat, pos_rank=pos_rank.to(device), reduction=config.reduction, weighted=config.weighted)
            else:
...
if __name__ == "__main__":
    ...
    loglogs = '_'.join((config.data, model_list[config.model].__name__, sampler_list[config.sampler].__name__, str(config.sample_size), str(config.pool_size), str(config.sample_num),timestamp))
